    ...
    def succAndProbReward(self, state, action):
        ...
        if deck is None:  # when there is no card in the deck
            ...
            def get_succ_reward(idx):
                new_card_sum = self.cardValues[idx] + state[0]  ## HINT: use self.cardValues  # what's the new sum of card values, when we take a new card?
        ...
        # Peek implementation ----------------------------------------
            ...
            if peek_idx is None:
                ...
                for idx, num in enumerate(deck):
                    if num == 0:
                        continue
                    prob = num / num_all_cards #      card_sum, peek_idx, deck = state  
